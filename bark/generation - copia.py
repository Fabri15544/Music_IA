...
def preload_models(
    ...
    if _grab_best_device() == "cpu" and (
        ...
        print(f"[GPU_DISABLED] CPU: {cpu_info} NUCLEOS: {num_cores}")
    _ = load_model(
...
def generate_text_semantic(
    ...
    block_texts = [text[i:i+block_size] for i in range(0, len(text), block_size)]
    block_results = []
    for block_text in block_texts:
        encoded_text = np.array(_tokenize(tokenizer, block_text)) + TEXT_ENCODING_OFFSET
        if OFFLOAD_CPU:
        ...
        if semantic_history is not None:
            ...
            semantic_history = np.full(256, SEMANTIC_PAD_TOKEN)
        x = torch.from_numpy(
            np.hstack([
        ...
        with torch.inference_mode():
            ...
            pbar_state = 0
            tot_generated_duration_s = 0  # Inicializar el contador de duración
            kv_cache = None
            for n in range(n_tot_steps):
                ...
                tot_generated_duration_s += 1 / SEMANTIC_RATE_HZ  # Incrementar la duración
...
def generate_coarse(
    ...
    use_kv_caching=False,
    desired_duration_seconds=120,  # Duración deseada del audio en segundos
    sampling_rate=44100  # Tasa de muestreo del audio
):
    ...
    device = next(model.parameters()).device
    # start loop
    desired_duration_steps = desired_duration_seconds * sampling_rate / COARSE_RATE_HZ
    n_window_steps = int(np.ceil(desired_duration_steps / sliding_window_len))
    assert n_window_steps > 0 and n_window_steps % N_COARSE_CODEBOOKS == 0
    ...
    with _inference_mode():
        ...
        for _ in tqdm.tqdm(range(n_window_steps), total=n_window_steps, disable=silent):
            ...
            for _ in range(sliding_window_len):
                if n_step >= desired_duration_steps:
                    continue
    ...
    del x_coarse_in
    assert len(gen_coarse_arr) == desired_duration_steps
...
def generate_fine(
    ...
    # Carga los modelos si aún no existen
    global models
    global models_devices
    ...
    in_arr = np.vstack(
        [
            x_coarse_gen,
            np.full((N_FINE_CODEBOOKS - n_coarse, x_coarse_gen.shape[1]), CODEBOOK_SIZE, dtype=np.int32),  # Relleno
        ]
    ).astype(np.int32)
    # Agregar historial si está disponible (máximo 512)
    if x_fine_history is not None:
        ...
        n_history = 0
    n_remove_from_end = max(0, 1024 - in_arr.shape[1])
    in_arr = np.pad(in_arr, ((0, 0), (0, n_remove_from_end)), mode='constant', constant_values=CODEBOOK_SIZE)
    
    # Podemos ser poco rigurosos sobre el bucle fraccionario y simplemente sobrescribir codebooks
    n_loops = max(0, int(np.ceil((x_coarse_gen.shape[1] - (1024 - n_history)) / 512))) + 1
    with torch.inference_mode():
        ...
        for n in tqdm.tqdm(range(n_loops), disable=silent):
            ...
            # Transferir información a model_in y convertir a numpy
            for nn in range(n_coarse, N_FINE_CODEBOOKS):
                in_arr[
